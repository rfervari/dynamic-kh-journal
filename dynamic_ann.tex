...
A well-established form of updating relational models is the elimination of worlds. This model update is interpreted as a truthful announcement indicating to all agents, in a fully public way, that a formula $\chi$ is the case~\cite{Plaza89:lopc,DELbook}. To describe the effects of this action, Public Announcement Logic (\PAL) extends the basic modal logic with a dynamic operator $[\chi]$. Given an $\lts$ $\modults=\tup{\W,\R,\V}$, the semantic interpretation of $[\chi]$ is usually defined as
\begin{spcenter}
  $\modults,w\models[\chi]\varphi \ \ \ \text{iff} \ \ \  \modults,w\models\chi \mbox{ implies } \modults_{\chi},w\models\varphi$,
\end{spcenter}
with the $\lts$ $\modults_\chi=\tup{\W_\chi,\R_\chi,\V_\chi}$ given by $\W_\chi = \set{w \mid \modults,w\models\chi}$, $(\R_\chi)_a = \R_a \cap \W_\chi^2$ for each $a\in\ACT$, and $\V_\chi(w) = \V(w)$ for all $w\in\W_\chi$.  In other words, $\modults_\chi$ is obtained by taking $\truthset{\modults}{\chi}$ as the new domain, then restricting relations and valuation accordingly. This operation can be naturally extended to $\ults$s by setting $\Unc_\chi = {\Unc}$ to indicate that the update does not affect indistinguishability sets.  

A remark might be useful here. In the original knowing how setting with semantics defined in terms of \lts{s}~\cite{Wang2016}, edges can be taken to encode the agents' \emph{epistemic take on their abilities}: the actions they consider possible, the preconditions required for their execution, and the effects they have (how a situation changes under the execution of a given action). As a result, in such a setting, updates on states and edges are epistemic updates. In the \ults-based semantics, however, the edges in an \ults stand for the actions available in the considered situation, their preconditions and effects, independently of the agents' knowledge about them. In short, the 
...
It can be shown that the update operator adds expressivity to $\KHilogic$ (a similar result was established in~\cite{Wang2016} for the original $\lts$-based setting).
...
\begin{proof}
  Consider the pointed \ultss $(\modults,w)$ and $(\modults', w')$ below, with $\Unc(i)=\Unc'(i)=\set{\set{a}}$, and with dashed lines indicating the nodes and edges removed by an update with $[p]$. 
\begin{center}
    \begin{tikzpicture}[->]
...
\begin{proof}
	Let $(\modults,w)$ and $(\modults', w')$ be the single agent pointed models depicted below, with $\Unc(i):=\set{\set{ab}}$ and $\Unc'(i):=\set{\set{a}}$:
	...
	\begin{center}
		...
		\end{tikzpicture}
\end{center}
The pointed models are $\KHilogic$-bisimilar (\Cref{def:bisim-khi}), hence, they satisfy the same formulas in $\KHilogic$. However, $\modults,w \not\models \gann{r}\khi(p,q)$ (given that $\modults,w \models r$ and $\annmodel{r},w \not\models \khi(p,q)$) whereas $\modults',w' \models \gann{r}\khi(p,q)$ (given that $\modults',w' \models r$ and $\annmodel{r}',w \models \khi(p,q)$).
	%
	% \begin{nscenter}
	%   \scalebox{.7}{
	% \begin{tikzpicture}[->, grow' = right, level/.style={sibling distance = 3em/#1}, level distance = 3.5em]
...
In the proposition above, both models $\modults$ and $\modults'$ satisfy the formula $\khi(p,q)$, with their respective witnesses being $\set{ab}$ and $\set{a}$. Still, as discussed, after eliminating worlds in which $r$ is false, this is no longer the case: the formula fails in $\annmodel{r}$ but holds in $\annmodel{r}'$. Notice that the witness in $\modults$ is a singleton set with a two-step plan, while the witness in $\modults'$ is a singleton set with a one-step plan (i.e., an atomic action). This makes a difference because, by removing intermediate steps (which exist in the first but not in the second), $\PAKHilogic$ can tell these two models apart. 

Now, recall the class $\cultsba$ introduced in~\Cref{def:class-m-one}. By restricting the class of models to $\cultsba$, one can still get agents with `the same abilities' while also stopping $\PAKHilogic$ from being able to tell $\KHilogic$-bisimilar models apart.  Indeed, the reduction axioms from~\Cref{tab:palaxiom} are valid in the class of models $\cultsba$. Since we can use these axioms to eliminate announcements by iteratively replacing the innermost occurrence of a $\gann{\chi}$ modality, completeness for $\PAKHilogic$ over $\cultsba$ follows.
...
To establish the soundness of the reduction axioms, we first need to prove the following semantic properties.
...
\begin{lemma}\label{lem:palproperties} Let $\chi$ and $\varphi$ be $\PAKHilogic$-formulas and let $\modults$ be an arbitrary \ults.
	...
	\begin{proof}
 For Item 1, note that $w \in \truthset{\modults}{\gann{\chi}\varphi}$ iff $\modults,w \models \gann{\chi}\varphi$, and thus iff $\modults,w \models \chi$ implies $\annmodelchi,w \models \varphi$. The latter is equivalent to $\modults,w \models \neg\chi$ or $\annmodelchi,w \models \varphi$, so $w \in \truthset{\modults}{\neg\chi} \cup \truthset{\annmodelchi}{\varphi}$.
  
For Item 2, $w \in \truthset{\annmodelchi}{\varphi}$ iff $w \in \annsemantics{\W}{\chi} = \truthset{\modults}{\chi}$ and $w \in \truthset{\annmodelchi}{\varphi}$, and thus iff $w \in \truthset{\modults}{\chi} \cap \truthset{\annmodelchi}{\varphi}$. The previous set is in the form of $(A \cap B)$ so, by set reasoning, it is equal to $(A \cap (A^c \cup B))$. Thus, the statement above is equivalent to $w \in \truthset{\modults}{\chi} \cap (\truthset{\modults}{\neg\chi} \cup \truthset{\annmodelchi}{\varphi})$ which holds (the result of the first item) iff $w \in \truthset{\modults}{\chi} \cap \truthset{\modults}{\gann{\chi}\varphi}$, i.e., iff $w \in \truthset{\modults}{\chi \wedge \gann{\chi}\varphi}$.
	\end{proof}

\begin{lemma}\label{lemma:palkh-valid}
	The reduction axioms from~\Cref{tab:palaxiom} are sound in $\cultsba$.
	\end{lemma}
	
	\begin{proof} We will focus only on axiom $\axm{RKh}$, while proving the validity of the rest can be done as in $\PAL$.
...
Since all the reduction axioms are sound, we can state the intended result. 

\medskip 

\begin{theorem}\label{th:palcomplete}
$\axset_{\khi}$ plus the reduction axioms for $\gann{\chi}$ in~\Cref{tab:palaxiom} form a sound and strongly complete axiomatization for $\PAKHilogic$ with respect to $\cultsba$.
\end{theorem}
...
It is worth noticing that, even in the class $\cultsba$, the first discussed modality, $[\chi]$, adds expressive power to $\KHilogic$. Thus, there are no reduction axioms to eliminate its occurrences in formulas. This follows from~\Cref{prop:pal-exp}, since the counterexample models in the proof belong to the class $\cultsba$. Therefore, the meaning of $[\chi]$ and $\gann{\chi}$ is different even in such a restricted class of models.
...
\end{corollary}
\begin{proof}
Let $\varphi$ be a formula of $\PAKHilogic$. By using the reduction axioms from~\Cref{tab:palaxiom} repeatedly, $\varphi$ can be translated (in a finite number of steps) into a formula $\varphi'$, such that $\varphi$ and $\varphi'$ are equivalent in the class $\cultsba$ (\Cref{lemma:palkh-valid}). 
...
\item If $\varphi'$ is not satisfiable over \lts{s} then, a fortiori, it is not satisfiable in $\cultsba$. %Then, $\varphi$ is not in $\cultsba$.
\end{inlineenum}
\end{proof}
