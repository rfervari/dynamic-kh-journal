...
\raul{we should mention that this filtration also fixes the one in the previous paper, as here it is easier to show there is indeed one filtration.}
Now we proceed to define the preliminaries for decidability. The strategy to be followed is a filtration technique that produces as a result a finite model (see e.g.,~\cite{mlbook}). 
...
Moreover, if $\Sigma$ is finite, the relation $\planequiv_\Sigma$ enables us to obtain a finite set of witnesses for the formulas $\khi(\psi,\varphi)\in\Sigma$, from which we also get that $\ACT^\Sigma$ and (together with $\modequiv_\Sigma$) $\W^f$ are finite.
The function $\Unc^f(i)$ is well-defined since is made only of singletons. Specially, if there is no $\plans \in \Unc(i)$ such that is a witness for some $\khi(\varphi,\psi) \in \Sigma$, $\Unc^f(i)\neq \emptyset$ still holds.
...
In fact, notice that they are not in conflict with the new conditions, as the actions used to talk about the $\kh$ dimension are disjoint from those to talk about the action execution dimension.
...
Thus, $\W^f$ and $\V^f$ are finite. Similarly, but with the number of $\khi(\psi,\varphi) \in \Sigma$ and $\mlbox{a} \varphi$ in each case, $\ACT^{\Sigma}$ is finite.
Thus, $\R^f$ and $\Unc^f$ are both finite. Thus, $\modults^f$ is a finite model.
\end{proof}
