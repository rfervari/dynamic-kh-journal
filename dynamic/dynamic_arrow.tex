...
\begin{textonuevo}
	Again, if we consider the $\AUL$ modality from the literature instead of the version introduced here, we increase the expressive power of the logic $\KHilogic$, even over the class $\cultsba$. This easily follows from~\Cref{prop:pal-exp} as for the case of $\PAL$, by taking the formula $[(p,p)]\khi(p,q)$ (or $[(p,a,p)]\khi(p,q)$ in the version that also specifies the edge name). Futher details are omitted here. 
\end{textonuevo}
