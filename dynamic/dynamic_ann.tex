...
Note the following regarding models $\modults$ and $\modults'$ in~\Cref{prop:exppal} above. First, they both make the formula $\khi(p,q)$ true, with their respective witnesses being $\set{ab}$ and $\set{a}$. But, as discussed, after eliminating worlds in which $r$ is false, this is no longer the case: the formula fails in $\annmodel{r}$ but holds in $\annmodel{r}'$. Notice that the witness in $\modults$ is a singleton set with a two-step plan, while the witness in $\modults'$ is a singleton set with a one-step plan (i.e., an atomic action). This makes a difference because, by removing intermediate steps, $\PAKHilogic$ can tell these two models apart. 
