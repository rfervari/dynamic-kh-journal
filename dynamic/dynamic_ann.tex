...
\begin{proof}
  Consider the pointed \ultss $(\modults,w)$ and $(\modults', w')$ below, with $\Unc(i)=\Unc'(i)=\set{\set{a}}$, and with dashed lines indicating the nodes and edges an update with $p$ removes. 
\begin{center}
    \begin{tikzpicture}[->]
...
\begin{lemma}\label{lemma:palkh-valid}
	...
	\begin{proof} We will focus only on axiom $\axm{RKh}$; proving the validity of the rest can be done as in $\PAL$.
