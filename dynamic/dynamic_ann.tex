...
%that arises from taking $\truthset{\modults}{\chi}$ as the new domain, and with the relations and the valuation restricted accordingly .
...
\begin{proof}
  ...
  $\Unc(i)=\Unc'_i=\set{\set{a}})$ below are bisimilar and hence
...
% Nohe first option (i.e., the standard \PAL approach) follows an `intersection' approach : the states that will be $a$-reachable from a given state $w$ are those that were $a$-reachable from $w$ \emph{and} satisfied $\chi$ at $\modults$. The second option (model $\annmodelchi$) follows a `subset' approach: the states that will be $a$-reachable from a given state $w$ are \emph{all} those that were $a$-reachable from $w$ \emph{if they all satisfied $\chi$ at $\modults$}, and \emph{none} otherwise.
...
%\footnote{Formally, given $U \subseteq W$, define $N^{\subseteq U}(u) := \set{X \in N(u) \mid X \subseteq U}$ for each $u \in U$.}
...
\begin{proof}
	Let $\modults$ and $\modults'$ be the single agent models depicted below (states and edges depicted with dashed lines are those removed in $\annmodel{r}$ and $\annmodel{r}'$, respectively), with $\Unc(i):=\set{\set{ab}}$ and $\Unc'_i:=\set{\set{a}}$:
...
\begin{proof}
The result follows by correctness of the axioms and rule from~\Cref{tab:palaxiom} (which enables us to eliminate all the occurences of a $\gann{\chi}$ modality), together with the fact that the system in~\Cref{tab:khiaxiom} is also complete with respect to $\cultsba$ (see the proof in~\cite{AFSVQ21,AFSVQ23report}).
\end{proof} \raul{les parece dejar esto asi? o prefieren pongamos la prueba de correccion de los axiomas?}
...
\end{corollary}
\begin{proof}
Given a $\varphi$ from $\PAKHilogic$, it can be translated to an equivalent formula $\varphi'$ from $\KHilogic$ in $\cultsba$ (\Cref{tab:palaxiom}) in a finite amount of steps.
...
After that, there are two possible results:
\begin{inlineenum}
\item If $\varphi'$ is satisfiable, then it is in $\cultsfnu$ (\Cref{cor:satcultsker}).
As $\cultsfnu \subseteq \cultsba$, since each $\Unc(i)$ has singleton sets of actions, then $\varphi$ is satisfiable in $\cultsba$.
\item If $\varphi'$ is not satisfiable, then neither it is in $\cultsba$. Then, $\varphi$ is not in $\cultsba$.
\end{inlineenum}
Thus, there is a procedure that computes satisfiability $\PAKHilogic$ over $\cultsba$. \raul{I do not understant the proof, a formula is not in a class of models}
\end{proof}
