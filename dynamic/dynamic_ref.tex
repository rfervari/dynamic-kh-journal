...
\begin{itemize}
\item $\modults,u \models \psi$ and $\modults,v \models \varphi$;
...
\node (m) {$\modults$:};
\node [state, right = 0.4cm of m, label=below:$u$] (u) {$\psi$};
\node [state, right = of u, label=below:$v$] (v) {$\varphi$};
...
However, $\modults \models \refdiam{\plan_1}{\plan_2}\khi(\psi,\varphi)$ as $\splitstr{\set{\set{\plan_1,\plan_2}}}{\set{\set{\plan_1},\set{\plan_2}}}{\plan_1}{\plan_2}$ and $\set{\plan_1}$ acts as a witness.
\end{proof}
...
\begin{proof}
Since the refinement modality induces an epistemic change, propositional variables come unaffected.
Thus, $p\to\refdiam{a}{b}p$ is a valid formula, for every $a,b\in\ACT$. Consider the model below, with $\Unc(i):=\set{\set{a,b}}$:
