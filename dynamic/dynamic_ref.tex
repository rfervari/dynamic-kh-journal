One of the advantages of \ults{s} is that they allow a  natural
representation of both the actions that affect the abilities of an agent, and her epistemic state. In an
\ults, the crucial epistemic component is the sets $\Unc(i)$,
...
For $\plans \in S$ and $\plans=\plans_1\uplus\plans_2$, define  $\lmodel{S}{\plans}{\set{\plans_1,\plans_2}} \subseteq 2^{\ACT^*}$ as the result of refining~$\plans$ through $\set{\plans_1,\plans_2}$, i.e., $\lmodel{S}{\plans}{\set{\plans_1,\plans_2}} := (S\setminus\set{\plans})\cup\set{\plans_1,\plans_2}$.
\end{definition}
...
\begin{proof}
\Cref{itm:distref} follows from the standard $\forall$ pattern in the semantics of $\refbox{\plan_1}{\plan_2}$.
\Cref{itm:necessitationref} holds because the structure resulting from the operation is an \ults.
The seriality of $\splitstr{}{}{\plan_1}{\plan_2}$ ensures \Cref{itm:serialityref}.
\end{proof}

This dynamic modality preserves the previous knowledge of an agent, 
and it also can generate new knowledge, as shown above.

\medskip

\begin{proposition}\label{prop:ref-preserves-gains}
Let $\varphi,\psi$ be propositional formulas and $\plan_1,\plan_2\in\ACT^*$ such that $\plan_1\neq\plan_2$. Then,
\begin{enumerate}
...
\node [state, right = of u, label=below:$v$] (v) {$\psi$};
...
% However, properties as \Cref{itm:preservesknowledge} are not true for arbitrary formulas.
\raul{sabemos que item 1 no vale para forms arbitrarias, pero no sabemos nada de item 2. Decimos algo?  ver ej. siguiente}
\medskip
...
The models are $\KHilogic$-bisimilar, thus they satisfy the same formulas in $\KHilogic$ (in particular, $\neg\khi(p,q)$). But, $\modults,w \not\models \refdiam{a}{b}\khi(p,q)$ since $\splitstr{\Unc(i)}{\Unc(i)}{a}{b}$, % then $\Unc=\overline{\Unc}$,
...
\begin{proof}
The formula $p\to\refdiam{a}{b}p$ is a valid formula, for every $a,b\in\ACT$, since $\refdiam{a}{b}$ do not affect propositional symbols. Consider the model below, with $\Unc(i):=\set{\set{a,b}}$:
...
Therein, basic hybrid logic machinery is used to get axiom systems. This approach seems difficult to apply in our setting, as the language is unable to talk about the actual plans witnessing a formula. Thus, we will use different ideas to tackle this problem in~\Cref{sec:extension}.
