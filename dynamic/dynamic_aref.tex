...
$\lh{\psi}{\varphi}_i\chi$ is a ternary modality expressing that agent $i$ is able to learn how to achieve $\varphi$ given $\psi$, and that after this learning operation takes place, $\chi$ holds.
...
Let us go back to discussing the properties of $\AReflogic$, and their impact in obtaining complete axiomatizations.  $\arefdiam$ is normal and serial modality, that satisfies natural properties 
of Monotonicity and Weakening, but does not validate Positive nor Negative Introspection. 
...
It is also easy to see that Monotonicity and Weakening hold. For Item 6,  consider the model $\modults = \tup{\W,\R,\Unc,\V}$ depicted below, with  $\Unc(i):=\set{\set{a,b,c}}$.
\begin{center}
